...
class TestParser {
    ...
    private function genericCellsRecognized() {
        ...
        $items[] = array(123, new Meeting('21.2,7.3,21.3,4.4,18.4,2.5,16.5,30.5,13.6,', '5', 'В-206', 'Экономика недвижимости', 'лек.', 'Иевлева', 'ВЭМ-413'));
        $items[] = array(145, new Meeting('22.2,8.3,22.3,5.4,19.4,3.5,17.5,31.5,14.6,', '2', 'В-111', 'Деловой ин.яз.', 'пр.', 'Хван', 'ВЭМ-5'));
        $items[] = array(70, new Meeting('14.2,28.2,14.3,28.3,11.4,25.4,9.5,23.5,6.6,', '2', 'В-209', 'Теор.планир.эксп.', 'лаб.', 'Короткова', 'ВВТ-406', '1п/г 2п/г'));
        $items[] = array(65, new Meeting('13.2,27.2,13.3,27.3,10.4,24.4,8.5,22.5,5.6,', '1', 'Б-104', 'БЖД', 'лек.', 'Александрина', 'ВВТ-406'));
        $items[] = array(57, new Meeting('11.2,25.2,11.3,25.3,8.4,22.4,6.5,20.5,3.6,', '1', 'В-206', 'Вып.выпуск.раб.', 'лек.', 'Рыбанов', 'ВВТ-406'));
    ...
    private function customDateCellsRecognized() {
        ...
        $items[] = array(146, new Meeting('24.02, 24.03, 21.04, 19.05', '4', 'В-204', 'Теория кризисного управления', 'пр.', 'Гаврилова', 'ВЭМ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function doubleLineTitleRecognized() {
        ...
        $items[] = array(159, new Meeting('18.2,4.3,18.3,1.4,15.4,13.5,27.5,10.6,', '2', 'А-29', 'Комп.мет.и инф.сист.в техн.синт. пер.полим.', 'лаб.', 'Александрина', 'ВТПЭ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function complexDisciplinesRecognized() {
        ...
        $items[] = array(170, new Meeting('14.02,14.03,11.04,6.06', '2', 'Б-008', 'Теор. и эксп. мет. иссл. в химии', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(173, new Meeting('28.02,28.03,25.04,23.05', '3', 'Б-008', 'Рецептуростр. полим. композ', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(96, new Meeting('23.03,6.04', '0', 'А-32', 'Хим.реакторы', 'лек.', 'Бутов', 'ВЭ-411'));
        $items[] = array(100, new Meeting('13.2,27.2,13.3,27.3,10.4,24.4,8.5,22.5,5.6,', '4', 'Б-207', 'Химия', 'лаб.', 'Перевалова', 'ВЭ-411', '1 п/г'));
        $items[] = array(98, new Meeting('13.2,27.2,13.3,27.3,10.4,24.4,8.5,22.5,5.6,', '3', 'А-16', 'Комп.графика', 'лаб.', 'Саразов А.В.', 'ВЭ-411'));
        $items[] = array(92, new Meeting('18.02,18.03,15.04,13.05', '0', 'Д-202', 'Концепц.совр.естествозн', 'лек.', 'Перевалова', 'ВХТ-401'));
        $items[] = array(32, new Meeting('4.03,1.04,29.04,27.05', '0', 'Б-207', 'Концепц.совр.естествозн', 'пр', 'Перевалова', 'ВХТ-401'));
        $items[] = array(94, new Meeting('21.02,21.03,18.04', '0', 'В-111', 'Управление снабж.и сбытом', 'лек.', 'Чеботарев', 'ВХТ-401', 'по 3ч.'));
        $items[] = array(34, new Meeting('7.03,4.04,16.05,30.05', '2', 'В-111', 'Управление снабж.и сбытом', 'пр.', 'Чеботарев', 'ВХТ-401', 'по 3ч.'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
