...
class TestParser {
    ...
    private function genericCellsRecognized() {
        ...
        $items = array();
        $items[] = array(2, new Meeting('17.02,3.03,17.03,31.03,14.04,28.04,12.05,26.05,9.06', '2', 'А-12', 'АТПП', 'лек.', 'Чичилин', 'ВХТ-401'));
        $items[] = array(82, new Meeting('21.02,7.03,21.03,4.04,18.04,2.05,16.05,30.05,13.06', '5', 'В-206', 'Экономика недвижимости', 'лек.', 'Иевлева', 'ВЭМ-413'));
        $items[] = array(85, new Meeting('22.02,8.03,22.03,5.04,19.04,3.05,17.05,31.05,14.06', '2', 'В-111', 'Деловой ин.яз.', 'пр.', 'Хван', 'ВЭМ-5'));
        $items[] = array(146, new Meeting('14.02,28.02,14.03,28.03,11.04,25.04,9.05,23.05,6.06', '2', 'В-209', 'Теор.планир.эксп.', 'лаб.', 'Короткова', 'ВВТ-406', '1п/г 2п/г'));
        $items[] = array(131, new Meeting('13.02,27.02,13.03,27.03,10.04,24.04,8.05,22.05,5.06', '1', 'Б-104', 'БЖД', 'лек.', 'Александрина', 'ВВТ-406'));
        $items[] = array(102, new Meeting('11.02,25.02,11.03,25.03,8.04,22.04,6.05,20.05,3.06', '1', 'В-206', 'Вып.выпуск.раб.', 'лек.', 'Рыбанов', 'ВВТ-406'));
    ...
    private function physicalEducationCellsRecognized() {
        ...
        $items = array();
        $items[] = array(21, new Meeting('18.02,4.03,18.03,1.04,15.04,13.05,27.05,10.06', '1', '', 'Физическая культура', '', 'Хаирова', 'ВХТ-401'/*, '13.00-14.30'*/));
        $items[] = array(22, new Meeting('18.02,4.03,18.03,1.04,15.04,13.05,27.05,10.06', '2', '', 'Физическая культура', '', 'Хаирова', 'ВХТ-401'/*, '13.00-14.30'*/));
    ...
    private function customDateCellsRecognized() {
        ...
        $items[] = array(98, new Meeting('24.02,24.03,21.04,19.05', '4', 'В-204', 'Теория кризисного управления', 'пр.', 'Гаврилова', 'ВЭМ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function doubleLineTitleRecognized() {
        ...
        $items[] = array(29, new Meeting('18.02,4.03,18.03,1.04,15.04,13.05,27.05,10.06', '2', 'А-29', 'Комп.мет.и инф.сист.в техн.синт. пер.полим.', 'лаб.', 'Александрина', 'ВТПЭ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function parallelDisciplinesRecognized() {
        ...
        $items[] = array(58, new Meeting('20.02,6.03,20.03,3.04,17.04,1.05,15.05,29.05,12.06', '2', 'Б-306', 'Осн.терм.и кин. синтеза ВМС', 'лаб.', 'Пучков', 'ВХТ-401', '1/2 п/г'));
        $items[] = array(60, new Meeting('20.02,6.03,20.03,3.04,17.04,1.05,15.05,29.05,12.06', '2', 'Б-309', 'БЖД', 'лаб.', 'Шиповский', 'ВХТ-401', '2/1 п/г'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function complexDisciplinesRecognized() {
        ...
        $items[] = array(152, new Meeting('14.02,14.03,11.04,6.06', '2', 'Б-008', 'Теор. и эксп. мет. иссл. в химии', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(155, new Meeting('28.02,28.03,25.04,23.05', '3', 'Б-008', 'Рецептуростр. полим. композ', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(130, new Meeting('23.03,6.04', '0', 'А-32', 'Хим.реакторы', 'лек.', 'Бутов', 'ВЭ-411'));
        $items[] = array(139, new Meeting('13.02,27.02,13.03,27.03,10.04,24.04,8.05,22.05,5.06', '4', 'Б-207', 'Химия', 'лаб.', 'Перевалова', 'ВЭ-411', '1 п/г'));
        $items[] = array(135, new Meeting('13.02,27.02,13.03,27.03,10.04,24.04,8.05,22.05,5.06', '3', 'А-16', 'Комп.графика', 'лаб.', 'Саразов А.В.', 'ВЭ-411'));
        $items[] = array(164, new Meeting('18.02,18.03,15.04,13.05', '0', 'Д-202', 'Концепц.совр.естествозн', 'лек.', 'Перевалова', 'ВХТ-401'));
        $items[] = array(166, new Meeting('4.03,1.04,29.04,27.05', '0', 'Б-207', 'Концепц.совр.естествозн', 'пр', 'Перевалова', 'ВХТ-401'));
        $items[] = array(173, new Meeting('21.02,21.03,18.04', '3', 'В-111', 'Управление снабж.и сбытом', 'лек.', 'Чеботарев', 'ВХТ-401', 'по 3ч.'));
        $items[] = array(174, new Meeting('7.03,4.04,16.05,30.05', '2', 'В-111', 'Управление снабж.и сбытом', 'пр.', 'Чеботарев', 'ВХТ-401', 'по 3ч.'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
