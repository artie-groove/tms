...
class TestParser {
    ...
    private function genericCellsRecognized() {
        ...
        $items[] = array(116, new Meeting('21.2,7.3,21.3,4.4,18.4,2.5,16.5,30.5,13.6,', '5', 'В-206', 'Экономика недвижимости', 'лек.', 'Иевлева', 'ВЭМ-413'));
        $items[] = array(138, new Meeting('22.2,8.3,22.3,5.4,19.4,3.5,17.5,31.5,14.6,', '2', 'В-111', 'Деловой ин.яз.', 'пр.', 'Хван', 'ВЭМ-5'));
    ...
    private function customDateCellsRecognized() {
        ...
        $items[] = array(139, new Meeting('24.02, 24.03, 21.04, 19.05', '4', 'В-204', 'Теория кризисного управления', 'пр.', 'Гаврилова', 'ВЭМ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function doubleLineTitleRecognized() {
        ...
        $items[] = array(152, new Meeting('18.2,4.3,18.3,1.4,15.4,13.5,27.5,10.6,', '2', 'А-29', 'Комп.мет.и инф.сист.в техн.синт. пер.полим.', 'лаб.', 'Александрина', 'ВТПЭ-5'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
    ...
    private function complexDisciplinesRecognized() {
        ...
        $items[] = array(163, new Meeting('14.02,14.03,11.04,6.06', '2', 'Б-008', 'Теор. и эксп. мет. иссл. в химии', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(166, new Meeting('28.02,28.03,25.04,23.05', '3', 'Б-008', 'Рецептуростр. полим. композ', 'лаб.', 'Новопольцева', 'ВТПЭ-5', ', 009'));
        $items[] = array(89, new Meeting('23.03,6.04', '0', 'А-32', 'Хим.реакторы', 'лек.', 'Бутов', 'ВЭ-411'));
        $items[] = array(93, new Meeting('13.2,27.2,13.3,27.3,10.4,24.4,8.5,22.5,5.6,', '4', 'Б-207', 'Химия', 'лаб.', 'Перевалова', 'ВЭ-411', '1 п/г'));
        $items[] = array(91, new Meeting('13.2,27.2,13.3,27.3,10.4,24.4,8.5,22.5,5.6,', '3', 'А-16', 'Комп.графика', 'лаб.', 'Саразов А.В.', 'ВЭ-411'));
        if ( !$this->findMatch($title, $items) ) return false;
        $this->report($title, true);
        return true;
    }
